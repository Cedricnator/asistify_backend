...
export class GoogleCalendarRepository implements CalendarRepository {
  ...
  async createCalendar(summary: string): Promise<CalendarEntity> {
  ...
  async createDate(
    ...
    try {
      ...
      var saved = await calendar.events.insert({
        calendarId: calendarId,
        requestBody: { summary, start: startObj, end: endObj },
      });
      ...
      return new DateEntity(
        ...
        new Date(savedData.end!.dateTime!),
        timeZone,
      );
    } catch (error) {
      this.handleError(error);
  ...
  async updateDate(
    ...
    this.logger.log(
      `new startDatetime: ${startDate.toISOString()}, end: ${endDate.toISOString()}`,
    );
    try {
      var response = await calendar.events.update({
        calendarId,
        eventId,
        requestBody: { summary, start: startObj, end: endObj },
      });
