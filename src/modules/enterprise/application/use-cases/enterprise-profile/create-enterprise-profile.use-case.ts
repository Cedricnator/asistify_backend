...
import {
  SUSCRIPTION_REPOSITORY,
  type SuscriptionRepository,
} from 'src/modules/payments/domain/repositories/suscription.repository';
...
export class CreateEnterpriseProfileUseCase {
  ...
  async execute(
    ...
    const enterpriseProfile = await this.repo.create(params);

    if (params.isOwner) {
      const profile = await this.findProfileById.execute(params.profileId);
      const customerId = await this.suscriptionRepository.createCustomer(
        enterpriseProfile.id,
        profile,
      );
      await this.suscriptionRepository.createSuscription(
        customerId,
        params.membershipId!,
        params.enterpriseId,
      );
